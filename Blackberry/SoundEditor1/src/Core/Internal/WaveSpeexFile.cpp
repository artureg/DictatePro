...
const int WAVE_FRAME_SIZES[3][2][11] = { { { 8, 8, 8, 1, 1, 2, 2, 2, 2, 2, 2 }, // NB mono
...
bool WaveSpeexFile::openRead(const char *filePath) {
	...
	try {
		...
		try {
			...
			fread(&p_spxInfo, sizeof(WaveSpeexFileInfo), 1,
					p_riffFile->filep());
			p_riffFile->pop();
...
void WaveSpeexFile::setupInfo(int sampleRate, int bytesPerSample, int quality) {
	...
	p_coding.framesPerPacket =
			WAVE_FRAME_SIZES[p_coding.mode][p_fmtInfo.numberOfChannels - 1][p_coding.quality];
		...
		case 1: {
			...
			for (int i = 0; i < p_coding.framesPerPacket; i++) {
				...
				for (int j = 0; j < p_coding.frameSize; j++) {
					int offset = packetOffset * packet
							+ p_coding.framesPerPacket * i + j;
...
bool WaveSpeexFile::writeHeader() {
	...
	if ((fputs("RIFF", p_writeFile) == EOF)
			...
			|| (fwrite(&kSpeexInfoFullLength, sizeof(unsigned long), 1,
					p_writeFile) != 1)
			|| (fwrite(&p_fmtInfo, sizeof(WaveFMTInfo), 1, p_writeFile) != 1)
			|| (fwrite(&p_spxInfo, sizeof(WaveSpeexFileInfo), 1, p_writeFile)
					!= 1) || (fputs("data", p_writeFile) == EOF)
...
bool WaveSpeexFile::decodeToData(int offsetMilliSeconds,
		int durationMillSeconds, void* data, int* size) {
	...
	while ((packetIndex < length) && (!feof(p_riffFile->filep()))) {
		speex_bits_reset(&bits);
		int size = fread(cbits, 1, p_fmtInfo.bytesPerSample,
				p_riffFile->filep());
