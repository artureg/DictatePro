...
void SEAudioStreamPlayer::run() {
    ...
    if (position >= this->stream->getDuration()) {
    	position = 0;
    }
    ...
    while (is_running) { //positionS < durationS &&
		...
		if (FD_ISSET(
				snd_pcm_file_descriptor(pcm_handle, SND_PCM_CHANNEL_PLAYBACK),
				&wfds)) {
            if (pos < (int)stream->getDuration()) {
            	QByteArray bytes;
                stream->readData(bytes, pos, 100);
            	snd_pcm_plugin_write(pcm_handle, bytes.data(), bytes.length());
                pos += 100;
            } else {
            	allBuffered = true;
            }
            position = startPosition + QDateTime::currentMSecsSinceEpoch() - startTime;
            if (allBuffered) {
            	QThread::msleep(50);
            }
            emit positionChanged(position);
            if (position >= (int)stream->getDuration()) {
            	position = stream->getDuration();
            	emit positionChanged(position);
            	is_running = false;
            }
		}
