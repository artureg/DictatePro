...
bool WaveSpeexFile::encodeWavFile(const char* filePath, short quality) {
    ...
    p_fmtInfo.bytesPerSample = (short)(((WAVE_FRAME_SIZES[mode][numberOfChannels - 1][quality]*WAVE_BITS_PER_FRAME[mode][numberOfChannels - 1][quality]) + 7) >> 3);
    p_fmtInfo.bytesPerSecond = p_fmtInfo.bytesPerSample*50/WAVE_FRAME_SIZES[mode][numberOfChannels - 1][quality];
